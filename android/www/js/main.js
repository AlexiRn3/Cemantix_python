...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (window.Capacitor && StatusBar) {
        // Fait en sorte que la couleur de fond de la barre de statut (haut de l'écran) 
        // soit la même que le fond de l'application (votre crème).
        StatusBar.setBackgroundColor({ color: '#Fdfbf7' });
        // Définit le texte et les icônes (heure, batterie) comme sombres
    ...
    if (window.Capacitor) {
        App.addListener('backButton', () => {
            ...
            // Si l'utilisateur est sur la page de jeu, le bouton retour le ramène au Hub
            if (path.includes('/game')) {
                window.location.href = "/";
            } 
            // Si l'utilisateur est sur le Hub (page d'accueil), le bouton quitte l'application
